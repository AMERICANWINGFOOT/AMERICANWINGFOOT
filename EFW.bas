Rem A SIMPLE PROGRAM TO CONTROL A ZWO FILTER WHEEL IN QB64 WILL NEED FILE EFWfilter.DLL FROM ZWO
...
Do
    Do While _MouseInput '      Check the mouse status
        If _MouseButton(1) < 0 Then checklocation _MouseX, _MouseY: Rem LOOP UNTIL LEFT BUTTON HIT

    Loop
Loop Until InKey$ = Chr$(27): Rem ESC KEY TO END
